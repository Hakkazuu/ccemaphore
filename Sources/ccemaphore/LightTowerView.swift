...
struct LightTowerView: View {
    ...
    private func housing(_ g: Geo) -> some View {
        // AnyLayout (not a Group if/else) so the three lamp slots keep their view IDENTITY across an
        // orientation flip — the same views are re-laid-out V↔H rather than torn down and rebuilt, so the
        // change animates smoothly instead of popping (R10). macOS 13+, matching the deployment target.
        let layout = orientation == .vertical
            ? AnyLayout(VStackLayout(spacing: g.gap))
            : AnyLayout(HStackLayout(spacing: g.gap))
        return layout {
            slot(.red, g: g)
            slot(.yellow, g: g)
            slot(.green, g: g)
