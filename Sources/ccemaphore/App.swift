...
enum Entry {
    static func main() {
        ...
        if let i = args.firstIndex(of: "--remote-scan"), i + 1 < args.count {
            guard let host = RemoteHosts.resolve(args[i + 1]) else { print("no such remote host: \(args[i + 1])"); return }
            Task {
                switch await RemoteTranscriptPoller.pollOnce(host: host) {
                case .success(let sessions):
                    ...
                    if sessions.isEmpty { print("(no sessions found)") }
...
/// `MenuBarExtra`): left-click PINS the management panel open, right-click shows "show the light" +
