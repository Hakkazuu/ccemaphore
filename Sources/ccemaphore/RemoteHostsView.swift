...
private struct RemoteHostRow: View {
            ...
            HStack(spacing: 8) {
                ...
                if let testResult {
                    ...
                    case .success(let platform):
                        Text(L("remote.connection.success") + " (\(platform))")
                            .font(.system(size: 9))
    ...
    /// A real filled pill button (not plain text) — matches `sizeSegment`/`modeSegment` in
    /// `WidgetQuickSettingsView`, so a clickable action always LOOKS clickable.
